...
* ORC_archive_FILE: DFIR-orc archive in 7z format; This archive must contain an `NTFSInfo_detail[*].7z` archive generated by the [NTFSInfoDetail](https://github.com/DFIR-ORC/dfir-orc-config/blob/master/config/NTFSInfoDetail_systemdrive_config.xml) collection from DFIR-Orc;
* --csv_output (str): path to the results output (*Results_data.csv* by default). The output is a CSV document;
